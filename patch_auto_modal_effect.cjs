...
const effect = `  // Effect to handle selection validity
  useEffect(() => {
...
const regex = /const allowedSubjectIds = useMemo\([\s\S]*?\}, \[selectedClassIds, db\.classes, db\.curriculums, db\.subjects\]\);/;
