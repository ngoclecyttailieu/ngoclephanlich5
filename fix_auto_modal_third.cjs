...
const regexToRemove = /^\s*\/\/\s*Effect to handle selection validity\s*useEffect\(\(\) => \{[\s\S]*?db\.subjects\]\);\s*/m;
...
const insertRegex = /}, \[selectedClassIds, db\.classes, db\.curriculums, db\.subjects\]\);/;
code = code.replace(insertRegex, `$&` + effectCode);
